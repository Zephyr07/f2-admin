...
export class BrandListComponent implements OnInit {
  ...
  deleteBrand(i){
    i.remove().subscribe((a:any)=>{
      Metro.notify.create('Marque '+a.name+' supprimée', 'Succès', {timeout: 5000, cls: 'bg-noir fg-white'});
      this.getBrands();
    }, q => {
      if (q.data.error.status_code === 500) {
        Metro.notify.create('deleteBrand ' + JSON.stringify(q.data.error.message), 'Erreur ' + q.data.error.status_code, {cls: 'alert', keepOpen: true, width: 500});
      } else if (q.data.error.status_code === 401) {
        Metro.notify.create('Votre session a expiré, veuillez vous <a routerLink="/login">reconnecter</a>  ', 'Session Expirée ' + q.data.error.status_code, {cls: 'alert', keepOpen: true, width: 300});
      } else {
        Metro.notify.create('deleteBrand ' + JSON.stringify(q.data.error.errors), 'Erreur ' + q.data.error.status_code, {cls: 'alert', keepOpen: true, width: 500});
      }
    })
  }
